...
public class AdminUserService {
    ...
    public void banUser(Long userId, String reason, User admin) {
        ...
        // ========== BAN KASKADA: otkazivanje bukovanja + sakrivanje listinga ==========
        cascadeBanEffects(targetUser, reason);
    ...
    *   <li>Anonymize retained user row via GDPR tombstone path</li>
    ...
    /**
     * Kaskadni efekti bana:
     * 1. Otkazuje buduce bukovanja (PENDING_APPROVAL, ACTIVE, CHECK_IN_OPEN) — kao renter i kao owner
     * 2. Sakriva sve vlasnikove listinge (available=false)
     * 3. Salje notifikacije kontra-stranama
     * Za in-trip/dispute bukovanja loguje upozorenje (ne otkazuje automatski).
     */
    private void cascadeBanEffects(User bannedUser, String reason) {
        // Statusi pre fizicke primopredaje vozila — bezbedno za automatsko otkazivanje
        ...
        // 1a. Otkazivanje bukovanja gde je banovani korisnik renter
        List<Booking> renterBookings = bookingRepo.findByRenterIdAndStatusIn(
                bannedUser.getId(), cancellableStatuses);
        for (Booking booking : renterBookings) {
            cancelBanBooking(booking, reason);
            notifyCounterparty(booking.getCar().getOwner(), booking,
                    "Rezervacija #" + booking.getId() + " je otkazana jer je nalog gosta blokiran.");
        }
        log.info("Ban cascade: otkazano {} renter bukovanja za userId={}", renterBookings.size(), bannedUser.getId());

        // 1b. Otkazivanje bukovanja gde je banovani korisnik vlasnik vozila
        List<Booking> ownerBookings = bookingRepo.findByCarOwnerIdAndStatusIn(
                bannedUser.getId(), cancellableStatuses);
        for (Booking booking : ownerBookings) {
            cancelBanBooking(booking, reason);
            notifyCounterparty(booking.getRenter(), booking,
                    "Rezervacija #" + booking.getId() + " je otkazana jer je nalog vlasnika blokiran.");
        }
        log.info("Ban cascade: otkazano {} owner bukovanja za userId={}", ownerBookings.size(), bannedUser.getId());

        // 1c. Statusi nakon primopredaje ili u toku spora — admin mora rucno resiti
        List<BookingStatus> escalationStatuses = List.of(
        ...
        List<Booking> inTripAsRenter = bookingRepo.findByRenterIdAndStatusIn(
                bannedUser.getId(), escalationStatuses);
        List<Booking> inTripAsOwner = bookingRepo.findByCarOwnerIdAndStatusIn(
                bannedUser.getId(), escalationStatuses);
        if (!inTripAsRenter.isEmpty() || !inTripAsOwner.isEmpty()) {
            log.warn("BAN ALERT: userId={} ima {} aktivnih/dispute bukovanja (renter) i {} (owner). Admin mora rucno resiti.",
                    bannedUser.getId(), inTripAsRenter.size(), inTripAsOwner.size());
        }

        // 2. Sakrivanje svih vlasnickih listinga
        List<Car> cars = carRepo.findByOwnerId(bannedUser.getId());
        for (Car car : cars) {
            car.setAvailable(false);
            carRepo.save(car);
        }
        if (!cars.isEmpty()) {
            log.info("Ban cascade: sakriveno {} listinga za userId={}", cars.size(), bannedUser.getId());
        }
    }

    private void cancelBanBooking(Booking booking, String reason) {
        if (booking.getStatus() == BookingStatus.PENDING_APPROVAL) {
            ...
            cancellationSettlementService.beginFullRefundSettlement(
                    ...
                    "Ban kaskada: " + reason,
                    "ADMIN_BAN_FULL_REFUND"
            );
        }
    }

    private void notifyCounterparty(User recipient, Booking booking, String message) {
        ...
        try {
            ...
            log.warn("Neuspesno slanje notifikacije za ban kaskadu: recipientId={}, bookingId={}, error={}",
                    recipient.getId(), booking.getId(), e.getMessage());
        }
    }
