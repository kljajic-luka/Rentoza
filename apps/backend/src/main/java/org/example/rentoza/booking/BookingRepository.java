...
public interface BookingRepository extends JpaRepository<Booking, Long>, JpaSpecificationExecutor<Booking> {
    ...
    /**
     * Bukovanja gde je korisnik vlasnik vozila, filtrirano po statusima.
     * Koristi se za ban kaskadu — otkazivanje bukovanja na vlasnickim vozilima.
     */
    @Query("SELECT b FROM Booking b " +
           "WHERE b.car.owner.id = :ownerId " +
           "AND b.status IN :statuses")
    List<Booking> findByCarOwnerIdAndStatusIn(
            @Param("ownerId") Long ownerId,
            @Param("statuses") List<BookingStatus> statuses
    );
    ...
    // ========== TASK 6: VEHICLE NOT RETURNED DETECTION ==========

    /**
     * Pronalazi IN_TRIP bookings gde je zakazano vracanje proslo za vise od 24h,
     * a vehicleNotReturnedFlag jos nije postavljen.
     */
    @Query("""
           SELECT b FROM Booking b
           JOIN FETCH b.renter r
           JOIN FETCH b.car c
           WHERE b.status = 'IN_TRIP'
             AND b.vehicleNotReturnedFlag = false
             AND b.endTimeUtc IS NOT NULL
             AND b.endTimeUtc < :overdueThreshold
           ORDER BY b.endTimeUtc ASC
           """)
    List<Booking> findOverdueInTripBookings(@Param("overdueThreshold") Instant overdueThreshold);
}
