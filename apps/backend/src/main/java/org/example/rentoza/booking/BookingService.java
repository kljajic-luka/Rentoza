...
import org.example.rentoza.payment.ProtectionTierConfig;
...
public class BookingService {
    ...
    public BookingCreationResult createBooking(BookingRequestDTO dto, String renterEmail) {
        ...
        // ========================================================================
        // BOOKING SPAM CAP (Abuse Prevention)
        // ========================================================================
        // Ogranicenje: maksimalno 10 PENDING_APPROVAL bukinga po renteru.
        // Sprecava spam-bukiranje koje prezauzima kalendare vlasnika.
        // ========================================================================
        long pendingCount = repo.countByRenterIdAndStatus(
                renter.getId(), BookingStatus.PENDING_APPROVAL);
        if (pendingCount >= 10) {
            log.warn("Booking spam cap reached: userId={}, pendingCount={}", renter.getId(), pendingCount);
            throw new org.example.rentoza.exception.ValidationException(
                    "Imate " + pendingCount + " rezervacija na cekanju. " +
                    "Sacekajte da vlasnici odgovore pre kreiranja novih.");
        }
        ...
        // Apply protection tier multiplier (precise decimal representation)
        ProtectionTierConfig protectionTier = ProtectionTierConfig.fromString(booking.getInsuranceType());
        BigDecimal insuranceMultiplier = protectionTier.getPriceMultiplier();
        ...
        BigDecimal listingDeposit = car.getSecurityDepositRsd() != null
                ? car.getSecurityDepositRsd()
                : BigDecimal.valueOf(defaultDepositAmountRsd);

        // Apply protection tier deposit multiplier
        BigDecimal depositAmount = protectionTier.calculateEffectiveDeposit(listingDeposit);
        ...
        booking.setEffectiveDepositSnapshot(depositAmount);
    ...
    * - messagingAllowed: true for active lifecycle states and COMPLETED, false for cancelled/declined/expired/no-show
    ...
    private void sendPendingApprovalNotifications(Booking booking, User renter, Car car) {
        try {
            ...
            String renterMessage = String.format(
                    "Vaš zahtev za rezervaciju %s je poslat! Čeka se odobrenje vlasnika. (%s zastita%s, Period: %s)",
            ...
            String ownerMessage = String.format(
                    "Novi zahtev za rezervaciju %s od %s %s! (%s zastita%s, Period: %s)",
    ...
    private void sendInstantBookingNotifications(Booking booking, User renter, Car car) {
        try {
            ...
            String renterMessage = String.format("Vaša rezervacija za %s je potvrđena! (%s zastita%s, Period: %s)",
                    carInfo, insuranceInfo, refuelInfo, tripTimeInfo);
            notificationService.createNotification(CreateNotificationRequestDTO.builder()
                    .recipientId(renter.getId())
            ...
            String ownerMessage = String.format("Nova rezervacija za %s od %s %s (%s zastita%s, Period: %s)",
                    carInfo, renter.getFirstName(), renter.getLastName(), insuranceInfo, refuelInfo, tripTimeInfo);
            notificationService.createNotification(CreateNotificationRequestDTO.builder()
                    .recipientId(car.getOwner().getId())
                    .type(NotificationType.BOOKING_CONFIRMED)
