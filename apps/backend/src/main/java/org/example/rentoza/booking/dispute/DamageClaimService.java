...
public class DamageClaimService {
    ...
    public DamageClaimDTO createClaim(
        ...
        // H-4 FIX: Validate claimed amount does not exceed tier-aware coverage cap
        // Cap = effectiveDepositSnapshot + tier.maxPlatformCoverage
        ProtectionTierConfig tier = ProtectionTierConfig.fromString(booking.getInsuranceType());
        BigDecimal depositSnapshot = booking.getEffectiveDepositSnapshot() != null
                ? booking.getEffectiveDepositSnapshot()
                : (booking.getSecurityDeposit() != null ? booking.getSecurityDeposit() : BigDecimal.ZERO);
        BigDecimal maxCoverage = depositSnapshot.add(tier.getMaxPlatformCoverage());
        if (maxCoverage.signum() > 0 && claimedAmount.compareTo(maxCoverage) > 0) {
            throw new IllegalArgumentException(String.format(
                    "Iznos prijave (%.0f RSD) ne može biti veći od maksimalnog pokrića (%.0f RSD)",
                    claimedAmount.doubleValue(), maxCoverage.doubleValue()));
        }
        ...
        adminAlertService.alertDisputeFiled(claim);
    ...
    public DamageClaimDTO createGuestClaim(
        ...
        // Validate claimed amount does not exceed tier-aware coverage cap (mirrors createClaim)
        ProtectionTierConfig guestTier = ProtectionTierConfig.fromString(booking.getInsuranceType());
        BigDecimal guestDepositSnapshot = booking.getEffectiveDepositSnapshot() != null
                ? booking.getEffectiveDepositSnapshot()
                : (booking.getSecurityDeposit() != null ? booking.getSecurityDeposit() : BigDecimal.ZERO);
        BigDecimal guestMaxCoverage = guestDepositSnapshot.add(guestTier.getMaxPlatformCoverage());
        if (guestMaxCoverage.signum() > 0 && claimedAmount.compareTo(guestMaxCoverage) > 0) {
            throw new IllegalArgumentException(String.format(
                    "Iznos reklamacije (%.0f RSD) ne može biti veći od maksimalnog pokrića (%.0f RSD)",
                    claimedAmount.doubleValue(), guestMaxCoverage.doubleValue()));
        }
