...
public class BookingPaymentService {
     ...
     *
     * <p>P1-4: Fails safely if no valid deposit authorization or stored payment
     * instrument is available. Callers must ensure a payment instrument exists
     * before invoking this; never pass placeholder strings as fallback.
     */
    @Transactional
    public PaymentResult processDisputePayment(DamageClaim claim) {
        String depositAuthId = claim.getBooking().getDepositAuthorizationId();
        if (depositAuthId == null || depositAuthId.isBlank()) {
            log.error("[Payment] Cannot process dispute for claim {} on booking {} — " +
                    "no deposit authorization and no stored payment instrument available",
                    claim.getId(), claim.getBooking().getId());
            return PaymentResult.builder()
                    .success(false)
                    .errorCode("NO_PAYMENT_INSTRUMENT")
                    .errorMessage("No deposit authorization or payment instrument available " +
                            "for dispute charge on booking " + claim.getBooking().getId())
                    .status(PaymentStatus.FAILED)
                    .build();
        }
