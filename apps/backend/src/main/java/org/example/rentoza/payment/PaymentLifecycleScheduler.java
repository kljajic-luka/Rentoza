...
public class PaymentLifecycleScheduler {
    ...
    private static final String LOCK_DISPUTE_SLA    = "payment.scheduler.dispute.sla";
    private static final String LOCK_VEHICLE_OVERDUE = "payment.scheduler.vehicle.overdue";
    ...
    // ========== JOB: VEHICLE NOT RETURNED DETECTION ==========

    /**
     * Proverava IN_TRIP bookings gde je zakazano vracanje proslo za vise od 24h.
     * Za svaki: postavlja vehicleNotReturned flag, suspenduje nalog zakupca, salje admin alert.
     */
    @Scheduled(cron = "0 0 * * * *") // Every hour
    public void checkVehicleNotReturned() {
        if (!schedulerLockStore.tryAcquireLock(LOCK_VEHICLE_OVERDUE, LOCK_TTL_HOURLY)) {
            log.debug("[PaymentScheduler] checkVehicleNotReturned — lock held, skipping");
            return;
        }
        try {
            Instant overdueThreshold = Instant.now().minus(24, ChronoUnit.HOURS);
            List<Booking> overdue = bookingRepository.findOverdueInTripBookings(overdueThreshold);
            if (overdue.isEmpty()) {
                return;
            }

            log.warn("[PaymentScheduler] Vehicle not returned: {} booking(a) sa prekoracenjem od 24h", overdue.size());
            for (Booking booking : overdue) {
                try {
                    ...
                    renter.setRegistrationStatus(RegistrationStatus.SUSPENDED);
                    userRepository.save(renter);

                    adminAlertService.alertVehicleNotReturned(booking);

                    log.warn("[PaymentScheduler] Booking {} flagged — renter {} suspended",
                            booking.getId(), renter.getId());
                } catch (Exception e) {
                    log.error("[PaymentScheduler] Greska pri obradi overdue booking {}: {}",
                            booking.getId(), e.getMessage(), e);
                }
            }
        } finally {
            schedulerLockStore.releaseLock(LOCK_VEHICLE_OVERDUE);
        }
    }

    // ========== JOB: DISPUTE SLA BREACH DETECTION ==========

    /**
     * Provera sporova koji su probili SLA od 48h bez resenja.
     * Salje admin alert za svaki pronadjeni spor i azurira lastSlaAlertSentAt.
     * Alert cooldown: 24h izmedju ponovljenih alertova za isti spor.
     */
    @Scheduled(cron = "0 45 */6 * * *") // Every 6 hours at :45 past
    public void checkDisputeSla() {
        if (!schedulerLockStore.tryAcquireLock(LOCK_DISPUTE_SLA, LOCK_TTL_SIXHRLY)) {
            log.debug("[PaymentScheduler] checkDisputeSla — lock held, skipping");
            return;
        }
        try {
            Instant now = Instant.now();
            Instant slaThreshold = now.minus(48, ChronoUnit.HOURS);
            Instant alertCooldown = now.minus(24, ChronoUnit.HOURS);

            List<DamageClaim> breached = damageClaimRepository.findSlaBreachedClaims(slaThreshold, alertCooldown);
            if (breached.isEmpty()) {
                return;
            }

            log.warn("[PaymentScheduler] Dispute SLA breach: {} spor(ova) starijih od 48h bez resenja", breached.size());
            for (DamageClaim claim : breached) {
                adminAlertService.alertDisputeSlaBreached(claim);
                claim.setLastSlaAlertSentAt(now);
                damageClaimRepository.save(claim);
            }
        } finally {
            schedulerLockStore.releaseLock(LOCK_DISPUTE_SLA);
        }
    }
