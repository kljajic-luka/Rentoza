...
public class BookingEligibilityDTO {
    ...
    /**
     * Telefon nije verifikovan — potreban OTP pre rezervacije.
     */
    public static BookingEligibilityDTO phoneNotVerified() {
        return BookingEligibilityDTO.builder()
            .eligible(false)
            .blockReason(EligibilityBlockReason.PHONE_NOT_VERIFIED)
            .message("Phone number must be verified before booking")
            .messageSr("Broj telefona mora biti verifikovan pre rezervacije")
            .actionUrl("/users/profile?verifyPhone=1")
            .actionLabel("Verifikujte telefon")
            .build();
    }
