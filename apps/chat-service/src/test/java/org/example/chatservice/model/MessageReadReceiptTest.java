...
class MessageReadReceiptTest {
    ...
    void testCreateViaFactory() {
        ...
        assertThat(receipt.getMessageId()).isEqualTo(1L);
        assertThat(receipt.getUserId()).isEqualTo(100L);
        // create() explicitly sets readAt to LocalDateTime.now() to satisfy NOT NULL constraint
    ...
    @Test
    @DisplayName("should use Lombok @Data annotation (verified via generated getters/setters)")
    void testLombokDataAnnotation() {
        // Lombok @Data has RetentionPolicy.SOURCE so it's not visible at runtime.
        // Verify its effect: getters, setters, equals, hashCode, toString are generated.
        Class<?> entityClass = MessageReadReceipt.class;

        // Getter for messageId is generated by @Data
        assertThat(java.util.Arrays.stream(entityClass.getMethods())
                .anyMatch(m -> m.getName().equals("getMessageId") && m.getParameterCount() == 0))
                .isTrue();
        // Setter for messageId is generated by @Data
        assertThat(java.util.Arrays.stream(entityClass.getMethods())
                .anyMatch(m -> m.getName().equals("setMessageId") && m.getParameterCount() == 1))
                .isTrue();
    }

    @Test
    @DisplayName("should use Lombok @Builder annotation (verified via generated builder() method)")
    void testLombokBuilderAnnotation() {
        // Lombok @Builder has RetentionPolicy.SOURCE so it's not visible at runtime.
        // Verify its effect: a static builder() method is generated.
        Class<?> entityClass = MessageReadReceipt.class;
