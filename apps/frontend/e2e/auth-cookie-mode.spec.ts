 ...
 * - Test kredencijali se citaju iz ENV varijabli
 */
...
test.describe('Cookie-Based Authentication E2E Tests', () => {
  ...
  test('should login successfully and set HttpOnly cookies', async ({ page }) => {
    ...
    const loginRequest = apiRequests.find((req) => req.url.includes('/api/auth/supabase/login'));
  ...
  test('should send X-XSRF-TOKEN header on POST requests (CSRF protection)', async ({ page }) => {
    ...
    await page.route('**/api/**', (route) => {
      ...
      if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method) && !request.url().includes('/auth/supabase/login')) {
        mutationRequest = {
          url: request.url(),
          method: method,
          headers: request.headers(),
  ...
  test('should handle token refresh and maintain session', async ({ page }) => {
    ...
    await page.route('**/api/auth/supabase/refresh', (route) => {
      refreshRequest = {
        url: route.request().url(),
        headers: route.request().headers(),
      };
    ...
    const refreshResponse = await page.request.post('http://localhost:8080/api/auth/supabase/refresh', {
...
test.describe('Cookie-Based Authentication - Edge Cases', () => {
  ...
  test('should handle expired cookies gracefully', async ({ page }) => {
    ...
    await Promise.race([
      ...
      page.waitForResponse((response) => response.url().includes('/api/auth/supabase/refresh'), { timeout: 5000 }),
    ]).catch(() => {
