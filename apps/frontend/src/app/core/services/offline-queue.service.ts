...
export interface QueuedFormSubmission<
  TType extends QueuedFormSubmissionType = QueuedFormSubmissionType,
> {
  id: string;
  bookingId: number;
  type: TType;
...
export function isHostCheckInSubmissionPayload(
  payload: unknown,
): payload is HostCheckInSubmissionDTO {
  return (
    isRecord(payload) &&
...
export function isHandshakeConfirmationPayload(
  payload: unknown,
): payload is HandshakeConfirmationDTO {
  return (
    isRecord(payload) &&
    isFiniteNumber(payload['bookingId']) &&
...
export class OfflineQueueService implements OnDestroy {
  ...
  async processFormQueue(
    ...
    try {
      ...
      this.logger.log(
        `[OfflineQueue] Form sync complete: ${synced.length}/${items.length} successful`,
      );
    } catch (error) {
  ...
  /**
   * Process all queues in dependency order: photos first, then form submissions.
   * This guarantees that all photo uploads (which the backend references by URL)
   * are complete before the form submission that depends on them is sent.
   */
  async processAllQueues(
    uploadFn: (item: QueuedUpload) => Promise<boolean>,
    submitFn: (item: QueuedFormSubmission) => Promise<boolean>,
  ): Promise<{ photosSynced: string[]; formsSynced: string[] }> {
    this.logger.log('[OfflineQueue] Processing all queues in dependency order (photos → forms)');

    const photosSynced = await this.processQueue(uploadFn);
    this.logger.log(`[OfflineQueue] Photo queue done: ${photosSynced.length} synced`);

    // Only process forms if all photos succeeded (no remaining items means all cleared)
    const remainingPhotos = this._queue().length;
    if (remainingPhotos > 0) {
      this.logger.warn(
        `[OfflineQueue] ${remainingPhotos} photos still pending — deferring form submissions`,
      );
      return { photosSynced, formsSynced: [] };
    }

    const formsSynced = await this.processFormQueue(submitFn);
    this.logger.log(`[OfflineQueue] Form queue done: ${formsSynced.length} synced`);
  ...
  private async loadFormSubmissions(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (droppedCount > 0) {
          this.logger.warn(
            `[OfflineQueue] Dropped ${droppedCount} invalid form submissions from IndexedDB`,
          );
        }
