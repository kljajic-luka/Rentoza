...
describe('booking.utils - Feature 11 Hardening', () => {
  ...
  describe('isBookingCompleted', () => {
    it('should return true for COMPLETED status', () => {
      expect(isBookingCompleted({ status: 'COMPLETED', endTime: new Date(now.getTime() + 86400000).toISOString() }))
        .toBe(true);
    });

    it('should return true if endTime is in the past', () => {
      expect(isBookingCompleted({ status: 'ACTIVE', endTime: new Date(now.getTime() - 86400000).toISOString() }))
        .toBe(true);
    });

    it('should return true for checkout statuses with past endTime (aligned with backend)', () => {
      expect(isBookingCompleted({ status: 'CHECKOUT_OPEN', endTime: new Date(now.getTime() - 86400000).toISOString() }))
        .toBe(true);
      expect(isBookingCompleted({ status: 'CHECKOUT_GUEST_COMPLETE', endTime: new Date(now.getTime() - 86400000).toISOString() }))
        .toBe(true);
      expect(isBookingCompleted({ status: 'CHECKOUT_HOST_COMPLETE', endTime: new Date(now.getTime() - 86400000).toISOString() }))
        .toBe(true);
    });
