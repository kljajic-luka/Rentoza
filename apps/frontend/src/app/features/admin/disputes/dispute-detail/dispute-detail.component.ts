...
export class DisputeDetailComponent implements OnInit {
  ...
  submitResolution(): void {
    ...
    const decisionLabel = formVal.decision === 'APPROVED' ? 'Odobri'
      : formVal.decision === 'REJECTED' ? 'Odbij'
      : 'Delimično odobri';
    const amountText = formVal.approvedAmount
      ? ` — ${formVal.approvedAmount} RSD`
      : '';

    const dialogRef = this.dialog.open(ConfirmDialogMatComponent, {
      data: {
        title: 'Potvrda odluke o sporu',
        message: `Odluka: ${decisionLabel}${amountText}. Da li ste sigurni? Ova radnja će obavestiti obe strane i ne može se poništiti.`,
        confirmLabel: 'Da, potvrdi odluku',
        cancelLabel: 'Otkaži',
        danger: formVal.decision === 'REJECTED',
  ...
  private executeResolution(): void {
    ...
    if (this.isCheckoutDispute()) {
      const formVal = this.resolutionForm.value;
      const checkoutDecisionMap: Record<string, 'APPROVE' | 'REJECT' | 'PARTIAL'> = {
        APPROVED: 'APPROVE',
        REJECTED: 'REJECT',
        PARTIAL: 'PARTIAL',
        MEDIATED: 'PARTIAL',
