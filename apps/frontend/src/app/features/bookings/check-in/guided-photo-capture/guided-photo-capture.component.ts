    ...
    <div class="guided-capture-container" [class.dark-mode]="isDarkMode()">
      ...
      <main class="capture-main">
        ...
        <div class="guidance-phase">
          ...
          <button mat-stroked-button (click)="openGallery()" class="gallery-fallback-button">
          ...
          <input
            type="file"
            accept="image/*"
            [id]="'gallery-input-' + guidance.photoType"
            (change)="onPhotoSelected($event)"
            hidden
          />
        </div>
...
export class GuidedPhotoCaptureComponent implements OnInit, OnDestroy, OnChanges {
  ...
  protected openCamera(): void {
    ...
    if (input) {
      // Listen for focus return to detect camera cancellation / permission denial.
      // When the user cancels the file picker or denies camera access, the input
      // fires no 'change' event. We detect this by waiting for the window to
      // regain focus (the file dialog is modal) and then checking if a file was selected.
      let photoWasSelected = false;

      const onceChange = () => { photoWasSelected = true; };
      input.addEventListener('change', onceChange, { once: true });

      const onFocusBack = () => {
        // Small delay: on some devices the change event fires slightly after focus
        setTimeout(() => {
          input.removeEventListener('change', onceChange);
          if (!photoWasSelected) {
            this.snackBar.open(
              'Fotografija nije izabrana. Proverite dozvole kamere u podešavanjima uređaja.',
              'OK',
              { duration: 5000 },
            );
          }
  ...
  protected openGallery(): void {
    ...
    const input = document.getElementById(`gallery-input-${guidance.photoType}`) as HTMLInputElement;
    if (input) {
      input.click();
    }
  }
  ...
  protected onPhotoSelected(event: Event): void {
    ...
    if (!file) {
      // File input change fired with no file (e.g., browser-level cancel)
      this.snackBar.open(
        'Fotografija nije izabrana. Pokušajte ponovo ili izaberite iz galerije.',
        'OK',
        { duration: 4000 },
      );
      return;
