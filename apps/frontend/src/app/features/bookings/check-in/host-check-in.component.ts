    ...
    <div class="host-check-in" [class.readonly-mode]="readOnly">
      ...
      @if (showGuidedCapture() && !readOnly && !status?.timingBlocked) {
        <app-guided-photo-capture
          ...
          [preCompletedPhotoTypes]="alreadyUploadedPhotoTypes()"
...
export class HostCheckInComponent implements OnInit, OnChanges, OnDestroy {
  ...
  /**
   * Photo types already uploaded to the server (state === 'complete').
   * Passed to GuidedPhotoCaptureComponent so it can skip already-uploaded slots
   * on resume after partial upload or network interruption.
   */
  readonly alreadyUploadedPhotoTypes = computed<CheckInPhotoType[]>(() => {
    const progress = this.checkInService.uploadProgress();
    const uploaded: CheckInPhotoType[] = [];
    progress.forEach((p) => {
      if (p.state === 'complete') {
        uploaded.push(p.photoType as CheckInPhotoType);
      }
    });
    return uploaded;
  });
  ...
  onGuidedCaptureComplete(submission: GuestCheckInPhotoSubmissionDTO): void {
    ...
    // Determine which photos are already uploaded (resume scenario)
    const alreadyUploaded = new Set(this.alreadyUploadedPhotoTypes());
    let skipped = 0;

    // Process each photo from the guided capture
    for (const photo of submission.photos) {
      // Skip already-uploaded photos to avoid redundant re-uploads on resume
      if (alreadyUploaded.has(photo.photoType as CheckInPhotoType)) {
        skipped++;
        continue;
      }
    ...
    const newUploads = submission.photos.length - skipped;
    const msg = skipped > 0
      ? `${newUploads} novih fotografija poslato, ${skipped} već otpremljeno.`
      : `${submission.photos.length} fotografija uspešno snimljeno!`;
