...
export class BookingDialogComponent implements OnInit {
  ...
  protected readonly protectionCost = signal(0);
  ...
  private calculatePrice(): void {
    ...
    // Insurance cost
    const insuranceMultiplier = this.getProtectionMultiplier(insuranceType);
    const insurance = base * (insuranceMultiplier - 1);
    this.protectionCost.set(insurance);

    // Per-tier effective deposit (matches backend ProtectionTierConfig)
    const depositMultiplier = this.getDepositMultiplier(insuranceType);
    const listingDeposit = 30000; // default deposit in RSD
    this.depositAmount.set(Math.round(listingDeposit * depositMultiplier));
  ...
  private getProtectionMultiplier(type: string): number {
  ...
  private getDepositMultiplier(type: string): number {
    switch (type?.toUpperCase()) {
      case 'STANDARD':
        return 0.5;
      case 'PREMIUM':
