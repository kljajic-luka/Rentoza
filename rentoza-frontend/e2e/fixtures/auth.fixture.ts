...
export function assertCookieFlags(cookie: Cookie, expectedFlags: {
  httpOnly?: boolean;
  secure?: boolean;
...
export async function performLogin(page: Page, email: string = TEST_USER.email, password: string = TEST_USER.password) {
...
export async function verifyAuthenticationState(page: Page) {
  ...
  expect(accessTokenCookie, 'User should be authenticated (access_token cookie present)').toBeDefined();
...
export async function verifyUnauthenticatedState(page: Page) {
  const accessTokenCookie = await getCookieByName(page, COOKIE_NAMES.ACCESS_TOKEN);
  expect(accessTokenCookie, 'User should NOT be authenticated (no access_token cookie)').toBeUndefined();
}
...
export const test = base.extend<AuthFixtures>({
  ...
  authHelpers: async ({ page }, use) => {
    const helpers = {
      ...
      logout: async () => {
        await page.click('button:has-text("Logout"), button:has-text("Odjavi se"), [data-testid="logout-button"]').catch(() => {
