...
@Component({
  ...
  styles: [`
    ...
    .label { font-weight: 500; color: var(--admin-muted, #666); }
    .value { color: var(--admin-strong, #000); }
    .badge {
      padding: 4px 8px;
      border-radius: 4px;
      font-size: 12px;
    ...
    .badge-success { background: rgba(16, 185, 129, 0.12); color: #10b981; border-color: rgba(16, 185, 129, 0.2); }
    .badge-warning { background: rgba(249, 115, 22, 0.12); color: #f97316; border-color: rgba(249, 115, 22, 0.2); }
...
export class CarApprovalDialogComponent {
  ...
  getStatusClass(status?: string): string {
    switch(status) {
      case ApprovalStatus.APPROVED: return 'badge-success';
      case ApprovalStatus.PENDING: return 'badge-warning';
      case ApprovalStatus.REJECTED: return 'badge-error';
  ...
  getSubmitLabel(): string {
    ...
    switch(action) {
      case 'APPROVE': return 'Odobri Vozilo';
      case 'REJECT': return 'Odbij Vozilo';
      case 'SUSPEND': return 'Suspenduj Vozilo';
