...
export class EmailConfirmComponent implements OnInit {
  ...
  private processEmailConfirmation(): void {
    ...
    if (!fragment) {
      ...
      this.handleError('Nedostaju parametri za potvrdu emaila. Pokušajte ponovo sa linkom iz emaila.');
      return;
    }
  ...
  private confirmWithBackend(accessToken: string, refreshToken: string | null): void {
    ...
    this.http.post<{ user: any; message: string }>(
      ...
      error: (err) => {
        ...
        const message = err.error?.error || err.error?.message || 'Potvrda emaila nije uspela. Pokušajte ponovo.';
