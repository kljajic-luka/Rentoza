...
import { FuelType, TransmissionType, Feature, CancellationPolicy, CAR_RENTAL_RULES, Car } from '@core/models/car.model';
...
export class AddCarWizardComponent implements OnInit {
  ...
  protected readonly basicInfoForm = this.fb.nonNullable.group({
    ...
    year: [new Date().getFullYear(), [Validators.required, Validators.min(1950), Validators.max(2050)]],
  ...
  protected async submitForm(): Promise<void> {
    ...
    // Validate all steps
    if (this.basicInfoForm.invalid || this.specificationsForm.invalid || this.policiesForm.invalid) {
  ...
  protected cancelWizard(): void {
    if (confirm('Da li ste sigurni da želite da odustanete? Svi uneti podaci će biti izgubljeni.')) {
      this.router.navigate(['/owner/cars']);
    }
  }
